...
const AUTH_BACKEND = authIsCapacitorApp
  ? AUTH_DEPLOY_BACKEND
  : (authIsLocalHost
    ? (localStorage.getItem("use_local_api") === "1" ? authLocalBackend : AUTH_DEPLOY_BACKEND)
    : AUTH_DEPLOY_BACKEND);
...
if (authIsLocalHost && localStorage.getItem("use_local_api") === "1") {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data && data.mailSent === false) {
          ...
          localStorage.setItem("verify_notice", "تم إنشاء الحساب بنجاح. تم إرسال رمز التفعيل إلى بريدك الإلكتروني. لو مش ظاهر راجع Spam / الرسائل غير المرغوب فيها.");
        }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (payload && payload.mailSent === false) {
          ...
          setFormSuccess("verifyError", `تم إرسال رمز جديد إلى بريدك الإلكتروني (استغرق ${elapsed} ثانية). لو مش ظاهر راجع Spam / الرسائل غير المرغوب فيها.`);
        }
      } else {
  ...
  forgotForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormSuccess("forgotError", "تم إرسال الرمز إلى بريدك الإلكتروني. لو مش ظاهر راجع Spam / الرسائل غير المرغوب فيها.");
        return;
      }
      const msg = await readError(res, "تعذر إرسال الكود");
