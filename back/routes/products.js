...
router.post("/", requirePermission("product.write"), upload.fields([{ name: "images", maxCount: 6 }, { name: "images[]", maxCount: 6 }, { name: "image", maxCount: 1 }]), async (req, res) => {
  try {
    const body = req.body;
...
// update product
router.put("/:id", requirePermission("product.write"), upload.fields([{ name: "images", maxCount: 6 }, { name: "images[]", maxCount: 6 }, { name: "image", maxCount: 1 }]), async (req, res) => {
  try {
    const product = await Product.findById(req.params.id);
    if (!product) return res.status(404).json({ error: "Product not found" });
